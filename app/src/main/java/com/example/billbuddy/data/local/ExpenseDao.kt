...
interface ExpenseDao {
    ...
    @Query(
        "SELECT * FROM expense_table WHERE expenseDate BETWEEN datetime('now', '-7 day') AND datetime('now', 'localtime')"
    )
    fun getWeeklyExpenses(): Flow<List<Expense>>

    @Query(
        "SELECT * FROM expense_table WHERE expenseDate BETWEEN datetime('now', '-1 month') AND datetime('now', 'localtime')"
    )
    fun getMonthlyExpenses(): Flow<List<Expense>>

    @Query(
        "SELECT * FROM expense_table WHERE expenseDate >= datetime('now', '-3 day') AND expenseDate < datetime('now', 'localtime') "
    )
    fun get3DayExpenses(): Flow<List<Expense>>

    @Query(
        "SELECT * FROM expense_table WHERE expenseDate >= datetime('now', '-14 day') AND expenseDate < datetime('now', 'localtime')"
    )
