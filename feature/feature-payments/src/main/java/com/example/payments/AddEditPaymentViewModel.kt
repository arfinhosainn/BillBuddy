...
class AddEditPaymentViewModel @Inject constructor(
    ...
    private val _paymentList = MutableStateFlow(PaymentDetailState())
    ...
    private fun getPayments() {
        paymentRepository.getAllPayments().onEach { result ->
            when (result) {
                is Resource.Success -> {
                    _paymentList.value = PaymentDetailState(payments = result.data ?: emptyList())
                }
                is Resource.Error -> {
                    _paymentList.value = PaymentDetailState(
